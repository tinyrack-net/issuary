...
export class Migration20260910120000_api_security extends Migration {
  override async up(): Promise<void> {
    ...
    this.addSql(
      `create table "auth_budget" ("id" varchar(255) not null primary key, "attempts" integer not null, "expires_at" timestamptz not null);`,
    );
    this.addSql(
    ...
    this.addSql(
      `alter table "user" add column "sessions_invalidated_at" timestamptz null;`,
    );
    this.addSql(
      `create table "browser_session" ("id" varchar(255) not null primary key, "data" jsonb not null, "revision" integer not null default 0, "expires_at" timestamptz not null);`,
    );
    this.addSql(
