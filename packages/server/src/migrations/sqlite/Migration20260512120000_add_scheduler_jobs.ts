...
export class Migration20260512120000_add_scheduler_jobs extends Migration {
  override up(): void | Promise<void> {
    this.addSql(
      `create table \`scheduled_jobs\` (\`id\` text not null primary key, \`created_at\` datetime not null, \`updated_at\` datetime not null, \`name\` text not null, \`enabled\` integer not null default true, \`cron\` text not null, \`next_run_at\` datetime null, \`last_run_at\` datetime null, \`last_success_at\` datetime null, \`last_error_at\` datetime null, \`last_error\` text null, \`locked_by\` text null, \`locked_until\` datetime null, \`run_count\` integer not null default 0, \`failure_count\` integer not null default 0) /* Persistent scheduled jobs and leases */;`,
    );
    this.addSql(
