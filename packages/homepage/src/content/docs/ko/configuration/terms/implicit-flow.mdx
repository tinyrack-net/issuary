...
`app.signup_implicit_terms` 항목에 사용자에게 표시할 안내 문구를 언어별로 설정해요. HTML 태그를 사용할 수 있어요.
...
:::note
안내 문구는 HTML을 지원하므로 `<a>` 태그로 약관 전문 링크를 포함할 수 있어요. `class="link"` 클래스를 사용하면 `tinyauth`의 기본 테마에 맞는 링크 스타일이 적용돼요.
:::
...
1. **로그인/회원가입 페이지**: `signup_implicit_terms`에 설정한 안내 문구가 표시돼요.
