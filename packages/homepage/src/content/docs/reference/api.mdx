...
For a complete, interactive API reference with request/response examples and a built-in API client, visit the **[Interactive API Docs](/api-reference)**.
...
The raw OpenAPI 3.0 specification is available as a JSON file:

- **Static spec**: [`/openapi.json`](/openapi.json) (bundled at build time)
- **Live spec** (when running the server): `GET /api/docs/json`
