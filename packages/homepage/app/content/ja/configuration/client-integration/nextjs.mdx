---
title: Next.js 連動
description: Next.js アプリケーションと Issuary 連動する
section: configuration
order: 29
---

このガイドは Next.js App Routerアプリケーションで `issuary`を OIDC プロバイダとして使用して認証を実装する方法について説明します。 Next.jsはサーバーサイドでシークレットを安全に保管できるため、 **機密クライアント (Confidential Client)** パターンを使ってください。
...
`issuary`の `config.yaml`に Next.js アプリを機密クライアントとして登録します。
...
サーバー起動時 `issuary`の OIDC Discoveryエンドポイントから設定を自動的にインポートします。
