...
この文書は現在 `issuary`の `config.yaml` 構造をtop-levelグループ基準でまとめたリファレンスです。

:::note
`frontend`と `frontend.html_variables`はstandalone専用の設定です。 `@tinyrack/issuary-standalone`のみこの値を解釈し、 `@tinyrack/issuary-server`の直接構成では使用しません。
:::
...
| アイテム | タイプ | デフォルト | 環境変数 | 説明 |
|------|------|--------|-----------|------|
| `icon_url` | `string` (URL) | 公式 Issuary アイコン | - | アプリアイコン URL |
| `logo_url` | `string` (URL) | - | - | アイコンとタイトルを置き換えるロゴ URL |
| `title` | `Record<string, string>` | 基本多言語タイトル | - | 言語別のタイトル |
| `subtitle` | `Record<string, string>` | 基本の多言語挨拶 | - | ログインのサブタイトル。空文字列で非表示 |
| `login_method_description` | `Record<string, string>` | 基本の多言語案内 | - | ログイン方法の案内。空文字列で非表示 |
...
| `path` | `string` | `/opt/issuary/database.db` | `DATABASE_PATH` | データベースファイルパス |
...
| `name` | `string` | `'issuary'` | `DATABASE_NAME` | データベース名 |
...
| `title` | `string` | `'Issuary API'` | - | OpenAPIドキュメントのタイトル |
| `description` | `string` | `'OpenID Connect Provider API'` | - | OpenAPI ドキュメントの説明 |
| `ui_title` | `string` | `'Issuary API Reference'` | - | ライブスカラー API リファレンスUIのブラウザページタイトル |
