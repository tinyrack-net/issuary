...
이 문서는 현재 `issuary`의 `config.yaml` 구조를 top-level 그룹 기준으로 정리한 레퍼런스예요.

:::note
`frontend`와 `frontend.html_variables`는 standalone 전용 설정이에요. `@tinyrack/issuary-standalone`만 이 값을 해석하고, `@tinyrack/issuary-server`의 직접 구성에서는 사용하지 않아요.
:::
...
| 항목 | 유형 | 기본값 | 환경 변수 | 설명 |
|------|------|--------|-----------|------|
| `icon_url` | `string` (URL) | 공식 Issuary 아이콘 | - | 앱 아이콘 URL |
| `logo_url` | `string` (URL) | - | - | 아이콘과 제목을 대체하는 로고 URL |
| `title` | `Record<string, string>` | 기본 다국어 제목 | - | 언어별 제목 |
| `subtitle` | `Record<string, string>` | 기본 다국어 인사말 | - | 로그인 부제목. 빈 문자열이면 숨김 |
| `login_method_description` | `Record<string, string>` | 기본 다국어 안내 | - | 로그인 방식 안내. 빈 문자열이면 숨김 |
...
| `path` | `string` | `/opt/issuary/database.db` | `DATABASE_PATH` | 데이터베이스 파일 경로 |
...
| `name` | `string` | `'issuary'` | `DATABASE_NAME` | 데이터베이스 이름 |
...
| `title` | `string` | `'Issuary API'` | - | OpenAPI 문서 제목 |
| `description` | `string` | `'OpenID Connect Provider API'` | - | OpenAPI 문서 설명 |
| `ui_title` | `string` | `'Issuary API Reference'` | - | 라이브 Scalar API 레퍼런스 UI의 브라우저 페이지 제목 |
