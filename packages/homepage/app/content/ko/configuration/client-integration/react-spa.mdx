---
title: React SPA 연동
description: React 단일 페이지 애플리케이션과 Issuary 연동하기
section: configuration
order: 30
---

이 가이드는 React SPA(Single Page Application)에서 `issuary`를 OIDC 제공자로 사용하여 인증을 구현하는 방법을 설명해요. SPA는 클라이언트 측 코드에서 시크릿을 안전하게 보관할 수 없으므로 **공개 클라이언트(Public Client)** 패턴과 PKCE를 사용해요.
...
`issuary`의 `config.yaml`에 React SPA를 공개 클라이언트로 등록해요. `client_secret`은 생략해요.
