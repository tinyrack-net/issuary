---
title: Next.js 연동
description: Next.js 애플리케이션과 Issuary 연동하기
section: configuration
order: 29
---

이 가이드는 Next.js App Router 애플리케이션에서 `issuary`를 OIDC 제공자로 사용하여 인증을 구현하는 방법을 설명해요. Next.js는 서버 사이드에서 시크릿을 안전하게 보관할 수 있으므로 **기밀 클라이언트(Confidential Client)** 패턴을 사용해요.
...
`issuary`의 `config.yaml`에 Next.js 앱을 기밀 클라이언트로 등록해요.
...
서버 시작 시 `issuary`의 OIDC Discovery 엔드포인트에서 설정을 자동으로 가져와요.
