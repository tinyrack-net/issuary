...
`frontend` and `frontend.html_variables` are standalone-only settings. They are parsed by `@tinyrack/issuary-standalone` and are not part of the server's declarative config contract.

## `branding`

| Field | Type | Default | Description |
|------|------|--------|------|
| `icon_url` | `string` (URL) | Official Issuary icon | App icon URL. |
| `logo_url` | `string` (URL) | - | Logo that replaces the icon and title. |
| `title` | `Record<string, string>` | Default localized title | Localized title keyed by language code. |
| `subtitle` | `Record<string, string>` | Default localized greeting | Login subtitle. An empty localized value hides it. |
| `login_method_description` | `Record<string, string>` | Default localized guidance | Login method guidance. An empty localized value hides it. |
...
| `title` | `string` | `'Issuary API'` | OpenAPI document title. |
| `description` | `string` | `'OpenID Connect Provider API'` | OpenAPI document description. |
| `ui_title` | `string` | `'Issuary API Reference'` | Browser page title for the live Scalar API reference UI. |
