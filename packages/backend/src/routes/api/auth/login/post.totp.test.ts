 ...
 * =============================================================================
 */
describe('POST /api/auth/login - TOTP Required Mode', () => {
  let app: AppType;
 ...
 * =============================================================================
 */
describe('POST /api/auth/login - TOTP Optional Mode', () => {
  let app: AppType;
  let services: ServiceContainer;
  let cleanup: () => Promise<void>;
 ...
 * =============================================================================
 */
describe('POST /api/auth/login - TOTP Disabled Mode', () => {
  let app: AppType;
  let services: ServiceContainer;
  let cleanup: () => Promise<void>;
 ...
 * =============================================================================
 */
describe('POST /api/auth/login - Email Verification + TOTP', () => {
  let app: AppType;
  let services: ServiceContainer;
  let cleanup: () => Promise<void>;
 ...
 * =============================================================================
 */
describe('POST /api/auth/login - Session State Verification', () => {
  let app: AppType;
  let services: ServiceContainer;
  let cleanup: () => Promise<void>;
  ...
  test('should not allow protected API access with pending2FASetup session', async () => {
    ...
    const sessionClient = testClient(app);
    const changePasswordRes = await sessionClient.api.user.password.$put(
      {
        json: {
  ...
  test('should complete login flow: TOTP setup -> verify -> full session', async () => {
    ...
    // Step 2: Setup TOTP
    const sessionClient = testClient(app);
    const setupRes = await sessionClient.api.user.totp.setup.$post(
      {},
      { headers: { Cookie: `session=${sessionCookie}` } },
    );
    ...
    const verifyRes = await sessionClient.api.user.totp.verify.$post(
      {
        json: { code: validCode },
      },
      { headers: { Cookie: `session=${sessionCookie}` } },
    ...
    const confirmRes = await sessionClient.api.user.totp.confirm.$post(
      {
        json: {},
      },
      { headers: { Cookie: `session=${sessionCookie}` } },
    ...
    const finalSessionRes = await updatedClient.api.user.session.$get(
      {},
