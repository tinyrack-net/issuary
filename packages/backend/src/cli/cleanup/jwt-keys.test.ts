...
describe('jwtKeysTask', () => {
  ...
  test('should skip when disabled in config', async () => {
    ...
    try {
      const result = await jwtKeysTask.run({
        fastify: disabledApp,
        dryRun: false,
        verbose: false,
      });
  ...
  test('should skip when jwt_key_rotation_enabled is false', async () => {
    ...
    try {
      const result = await jwtKeysTask.run({
        fastify: noRotationApp,
        dryRun: false,
        verbose: false,
      });

      expect(result.skipped).toBe(true);
      expect(result.message).toBe('JWT key rotation is disabled in app config');
    } finally {
  ...
  test('should clear service cache after rotation', async () => {
    ...
    // The task should call clearActiveKeyCache
