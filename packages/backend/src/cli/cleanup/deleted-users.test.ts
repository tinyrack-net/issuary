...
describe('deletedUsersTask', () => {
  ...
  test('should skip when disabled in config', async () => {
    ...
    try {
      const result = await deletedUsersTask.run({
        fastify: disabledApp,
        dryRun: false,
        verbose: false,
      });
  ...
  test('should cascade delete all related entities', async () => {
    ...
    await withMikroContext(app, async () => {
      ...
      const passkeyCount = await em.count(UserPasskeyEntity, { user: userId });
      expect(passkeyCount).toBe(0);

      const consentCount = await em.count(UserConsentEntity, { user: userId });
      ...
      const resetCount = await em.count(PasswordResetEntity, { user: userId });
