...
describe('OAuthTokenService', () => {
  describe('cleanupExpiredCodes', () => {
    ...
    test('should skip when disabled in config', async () => {
      ...
      try {
        const result = await disabledApp.oauthTokenService.cleanupExpiredCodes({
          dryRun: false,
        });
    ...
    test('should return message when nothing to clean', async () => {
      const result = await app.oauthTokenService.cleanupExpiredCodes({
        dryRun: false,
      });
    ...
    test('should not delete recently consumed codes within retention', async () => {
      ...
      try {
        ...
        const result = await retentionApp.oauthTokenService.cleanupExpiredCodes(
          {
            dryRun: false,
          },
        );
