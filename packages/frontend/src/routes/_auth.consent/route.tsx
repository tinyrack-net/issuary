...
function ConsentError(props: RouteErrorComponentProps) {
  ...
  return (
    <RouteErrorFallback
      ...
      onUnauthorized={() => {
        ...
        navigateDocument(buildAuthorizeUrl(search));
      }}
    />
  );
}

function Consent({ search }: { search: z.infer<typeof ConsentSearchSchema> }) {
  ...
  const consentMutation = useMutation({
    ...consentDecisionMutationOptions,
    onSuccess: (data) => {
      navigateDocument(data.redirect_url);
    },
  });
...
export function ErrorBoundary({ error }: Route.ErrorBoundaryProps) {
  return <NativeRouteErrorBoundary component={ConsentError} error={error} />;
}

export default function ConsentRoute({ loaderData }: Route.ComponentProps) {
  return (
    <RouteHydrationBoundary state={loaderData.dehydratedState}>
      <Consent search={loaderData.search} />
    </RouteHydrationBoundary>
  );
}
