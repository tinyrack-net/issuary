...
export class ApiHelpers {
  ...
  async login(email: string, password: string): Promise<LoginResponse> {
    const response = await this.request.post(`${API_BASE_URL}/api/v1/auth/login`, {
      data: { email, password },
    });
  ...
  async logout(): Promise<void> {
    const response = await this.request.post(`${API_BASE_URL}/api/v1/auth/logout`);
  ...
  async getSession(): Promise<AuthResponse> {
    const response = await this.request.get(`${API_BASE_URL}/api/v1/user/session`);
  ...
  async forgotPassword(email: string): Promise<void> {
    const response = await this.request.post(`${API_BASE_URL}/api/v1/auth/password/forgot`, {
      data: { email },
    });

    if (!response.ok()) {
      const error = await response.text();
      throw new Error(`Forgot password failed: ${response.status()} - ${error}`);
    }
  }
  ...
  async resetPassword(token: string, password: string): Promise<void> {
    const response = await this.request.post(`${API_BASE_URL}/api/v1/auth/password/reset`, {
      data: { token, password },
    });
  ...
  async verifyEmail(token: string): Promise<{ user: SessionUser }> {
    const response = await this.request.post(`${API_BASE_URL}/api/v1/auth/verify-email`, {
      data: { token },
    });

    if (!response.ok()) {
      const error = await response.text();
      throw new Error(`Email verification failed: ${response.status()} - ${error}`);
    }
  ...
  async resendVerificationEmail(email: string): Promise<void> {
    const response = await this.request.post(`${API_BASE_URL}/api/v1/auth/verify-email/resend`, {
      data: { email },
    });

    if (!response.ok()) {
      const error = await response.text();
      throw new Error(`Resend verification failed: ${response.status()} - ${error}`);
    }
  }
  ...
  async verifyTotpLogin(code: string): Promise<{ user: SessionUser }> {
    const response = await this.request.post(`${API_BASE_URL}/api/v1/auth/totp/verify-login`, {
      data: { code },
    });
  ...
  async setupTotp(): Promise<{ secret: string; qr_code: string }> {
    const response = await this.request.post(`${API_BASE_URL}/api/v1/user/totp/setup`);
  ...
  async verifyTotpSetup(code: string): Promise<{ recovery_codes: string[] }> {
    const response = await this.request.post(`${API_BASE_URL}/api/v1/user/totp/verify`, {
      data: { code },
    });

    if (!response.ok()) {
      const error = await response.text();
      throw new Error(`TOTP setup verification failed: ${response.status()} - ${error}`);
    }
  ...
  async confirmTotpSetup(): Promise<{ user: SessionUser }> {
    const response = await this.request.post(`${API_BASE_URL}/api/v1/user/totp/confirm`);
  ...
  async disableTotp(password: string): Promise<{ user: SessionUser }> {
    const response = await this.request.post(`${API_BASE_URL}/api/v1/user/totp/disable`, {
      data: { password },
    });
  ...
  async changePassword(currentPassword: string, newPassword: string): Promise<void> {
    const response = await this.request.post(`${API_BASE_URL}/api/v1/user/password/change`, {
      data: { current_password: currentPassword, new_password: newPassword },
    });

    if (!response.ok()) {
      const error = await response.text();
      throw new Error(`Change password failed: ${response.status()} - ${error}`);
    }
  }
  ...
  async setPassword(password: string): Promise<{ user: SessionUser }> {
    const response = await this.request.post(`${API_BASE_URL}/api/v1/user/password/set`, {
      data: { password },
    });
  ...
  async removePassword(password: string): Promise<{ user: SessionUser }> {
    const response = await this.request.post(`${API_BASE_URL}/api/v1/user/password/remove`, {
      data: { password },
    });

    if (!response.ok()) {
      const error = await response.text();
      throw new Error(`Remove password failed: ${response.status()} - ${error}`);
    }
  ...
  async deleteAccount(password?: string): Promise<void> {
    const response = await this.request.delete(`${API_BASE_URL}/api/v1/user/account`, {
