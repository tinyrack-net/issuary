...
| `auth.password.email_verification` | Email verification required after register/login | `tests/email-verification/register-email-verify.test.ts`, `tests/email-verification/login-email-verify.test.ts` |
| `auth.password.second_factor.required` | Setup 2FA routing and required setup flow | `tests/totp-required/register-totp-setup.test.ts`, `tests/dual-2fa/choose-2fa.test.ts`, `tests/passkey-required/passkey-flow.test.ts` |
...
| `auth.passkey.enabled` | Passkey setup/verify availability and profile section | `tests/passkey-required/passkey-flow.test.ts`, `tests/dual-2fa/choose-2fa.test.ts`, `tests/totp-optional/totp-optional.test.ts` |
