...
## PR #83 추가 수정: 인증 폐기와 로그인 완료의 경합 (2026-09-10)

- **MFA 대기의 인증 세대 덮어쓰기:** 세션 로드 후 역할 변경으로 인증을 폐기하고 비밀번호 로그인을 재개하면, 이전 MFA 완료 상태가 새 세대를 승계해 보호 API가 200을 반환했다. 인증 전환 시 세대가 다른 동일 사용자의 로그인·기억된 계정·재인증·선택 허용 항목을 제거한다. 다른 사용자의 유효한 인증은 유지하며 MFA/설정 승격의 세대 불일치는 401이다.
- **연결 해제 후 OAuth 로그인:** 기존 연결 조회와 사용자 조회 사이에서 연결 해제·비밀번호 재설정을 완료하면, callback이 새 세대의 로그인 세션을 발급했다. 연결과 사용자 세대를 한 번의 joined 조회로 확보하고 사용자 → 브라우저 세션 순으로 잠근다. 원래 연결·세대·OAuth state·세션 revision을 재검사한 후 제공자 토큰 갱신, 약관 판정, state 소비, 세션 발급을 함께 커밋한다. 기존 연결이 사라진 요청을 가입/자동 연결로 전환하지 않는다. 외부 제공자 통신은 잠금 밖에 둔다.
- GET/POST callback에 공통 경계를 적용한다. 폐기된 OAuth 증명은 기존 400 `OAUTH_SESSION_EXPIRED`, 폐기된 브라우저 세션은 401, DB 경합은 409이다. 저장 실패 시 제공자 토큰 갱신과 state 소비도 롤백한다.
- 재현 두 건은 수정 전 401 기대값에 실제 200으로 실패했다. `authentication-race.test.ts`는 역할·이메일·비밀번호 변경, 다른 계정 보존, 새 MFA 완료, GET/POST callback의 연결 해제/재설정/복합 경합과 세션 저장 실패를 검증한다. 세션 단위 테스트는 설정 대기 상태와 세대 불일치 승격도 검증한다.
- `distributed-security.test.ts`는 두 독립 프로세스의 IPC 동기화 지점으로 로그인 도중 재설정과 OAuth 증명 조회 후 연결 해제·재설정을 제어한다. 공유 SQLite와 격리 PostgreSQL에서 같은 검증을 수행한다. 프로세스 종료 테스트는 두 프로세스 경합 검증 뒤에 실행한다.
- 로컬 자원 제한에 따라 작은 회귀 테스트와 변경 파일 검사만 로컬에서 수행한다. PostgreSQL 보안 작업에 새 회귀 파일을 포함하고, 기존 Security/Chromium 및 프런트엔드 smoke 작업으로 정상 MFA·계정 선택·OAuth·form_post를 검증한다. 최종 커밋의 CI 결과는 PR 검증 기록에 연결한다.
- 공개 성공 응답과 DB 스키마 변경은 없고 추가 마이그레이션도 없다. 이번 수정은 이후의 잘못된 인증 발급을 차단한다. 이미 발급된 세션의 이력만으로 악용 여부를 구별할 수는 없다. 기존 PR 전체의 마이그레이션·재로그인 전환 요구사항은 그대로 적용된다.

## 추가 리뷰: 토큰 재발급·OAuth 자동 연결·client 복원 (2026-09-10)

### 재현과 수정

- **이메일 인증/재설정 재발급:** 이전 토큰을 발급 서버의 현재 시각으로 만료시키면 30초 뒤처진 소비 서버가 승인했다. 두 토큰에 `revoked_at`을 추가하고 값의 존재로 폐기 여부를 판정한다. 사용자 잠금 안에서 이전 토큰 폐기와 새 토큰 저장을 묶었으며, 소비·유효 토큰 집계·메일 재시도도 같은 조건을 검사한다. 발급 실패는 이전 토큰 폐기까지 롤백한다. 정상 이메일 자동 로그인과 MFA는 유지한다.
- **OAuth 자동 연결/신규 가입:** 기존 연결 로그인에만 적용하던 완료 트랜잭션을 자동 연결, 즉시 신규 가입, 명시적 약관 대기로 확장했다. 제공자 조회 후 연결/자동 연결/신규 가입 증명을 구분하고, 자동 연결 대상의 sub·세대·이메일을 잠금 안에서 재검사한다. 최초 증명이 무효화되면 다른 가입/연결 경로로 전환하지 않는다. GET/POST 모두 사용자·연결·약관·대기 토큰·OAuth state·브라우저 세션 변경을 함께 커밋한다. 세션 저장/삭제 실패는 DB 변경을 롤백한다.
- **client 삭제/복원:** 미사용 code가 발급 당시 client 세대를 보존하지 않아 복원 후 새 세대 토큰을 발급했다. code와 device에 `client_epoch`를 저장하고 현재 client 세대와 비교한다. client 삭제/복원과 code 교환, device 승인/거절/polling/교환은 같은 client 행 잠금을 사용한다. 복원 후 새로 시작한 흐름은 정상 처리하고 이전 흐름은 `invalid_grant`로 거절한다. device polling 오류는 polling 상태 커밋 후 반환하여 기존 제한을 유지한다.

### 전환과 제약

SQLite/PostgreSQL의 새 `Migration20260910220000_flow_revocation`을 추가했다. 구버전 서버·메일 작업자를 종료하고 마이그레이션한 뒤 동일 설정의 신버전을 시작한다. 이전 미사용 이메일/재설정 토큰은 폐기하고 pending/running 메일 작업은 종료·정제한다. 기존 code/device에는 유효 client 세대와 일치하지 않는 빈 세대를 부여하므로 진행 중 흐름은 다시 시작해야 한다. 소비 기록과 grant 폐기 기록은 삭제하지 않는다. 이번 추가 마이그레이션은 기존 브라우저 세션과 발급된 grant를 일괄 폐기하지 않는다. 이전 인증 세대 마이그레이션을 아직 적용하지 않은 설치에는 해당 마이그레이션의 재로그인 요구도 적용된다.

이전 링크 사용은 기존 400, 브라우저 세션 폐기는 401, DB 잠금 충돌은 409이며 자동 재실행하지 않는다. 공개 성공 응답과 번역 키는 유지하고 callback/token OpenAPI 설명을 보완했다. 이미 전송 중인 SMTP 메일의 회수와 외부 서비스의 오프라인 JWT 즉시 폐기는 보장하지 않는다.
...
### 추가 심층 리뷰: 발급 완료·관리자·client 인증·폐기·device 약관

`706498c7`에서 보관한 6개 재현은 수정 전 모두 실패했다. 다음 5개 원인을 수정했으며 `grant-completion-security.test.ts`의 정식 회귀에 포함한다.
...
| Device의 필수 약관 우회 | device 승인에 공통 필수 약관·버전 검사가 없었다. GET 및 POST 승인에서 검사하고 약관 미완료 시 303으로 이동하며 pending 상태를 유지한다. 약관 완료 뒤 같은 device 화면에서 별도로 승인한다. 일반 약관 동의 저장도 브라우저 인증을 재검사한다. 브라우저 승인 폼의 제출 주소를 `/oauth/device`로 지정해 user_code가 URL과 본문에 중복되어 입력 검사에서 거절되는 문제도 해결했다. 승인 HTML의 Referrer-Policy는 same-origin으로 설정해 브라우저 native POST가 Origin: null로 변하지 않게 하며, 외부 referrer 전송과 null origin의 CSRF 허용은 추가하지 않는다. | 직접 POST 우회 차단, 약관 버전 변경 반영, 약관 동의만으로 토큰 발급 불가, 복귀 후 정상 승인. |
