...
Use `data-testid` attributes for E2E test selectors to ensure stability across i18n changes and UI refactoring.
...
- `-alert` for alert/notification elements
...
| `/verify/totp/recovery` | `verify-totp-recovery-` |
| `/setup/totp` | `setup-totp-` |
| `/profile` | `profile-` |
| `/consent` | `consent-` |
| Profile modals | `modal-{modal-name}-` |

#### Examples
```tsx
// Input fields
<input data-testid="login-password-email-input" type="email" />
<input data-testid="register-password-input" type="password" />

// Buttons
<button data-testid="login-password-submit-btn" type="submit">Login</button>
<button data-testid="profile-logout-btn">Logout</button>

// Links
<a data-testid="login-password-forgot-link" href="/password/forgot">Forgot?</a>

// Error messages
<p data-testid="login-password-email-error" className="text-error">Invalid email</p>

// Indexed elements (e.g., PIN inputs)
<input data-testid="verify-totp-pin-0" />
<input data-testid="verify-totp-pin-1" />
```

#### Guidelines
- **Always use data-testid** for interactive elements tested by E2E
- **Avoid text-based selectors** (`hasText`, regex patterns) - they break on i18n changes
- **Avoid structural selectors** (`.locator('..')`) - they break on HTML changes
- **Keep stable href selectors** (`a[href="/login"]`) for navigation links when appropriate
- **Pass data-testid through reusable components** via props

### E2E Page Objects
- Located in `packages/frontend/e2e/pages/`
- Use `page.locator('[data-testid="..."]')` for element selection
